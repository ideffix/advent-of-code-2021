...
function sumVersionNumbers(package) {
  return package.subPackages.reduce((acc, el) => acc + sumVersionNumbers(el), package.version)
}

const packages = parsePackage(binary)

const result = sumVersionNumbers(packages)
