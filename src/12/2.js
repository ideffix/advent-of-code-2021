...
function traverse(start) {
  return start.edges.reduce((acc, e) => acc + numberOfPathsToEnd(e, new Set([start]), false), 0)
}

function numberOfPathsToEnd(vertex, visited, isSingleSmallCaveVisitedTwice) {
  if (vertex.name === 'end') return 1;
  if (vertex.name === 'start' || (isSingleSmallCaveVisitedTwice && vertex.isSmallCave && visited.has(vertex))) return 0;

  return vertex.edges.reduce((acc, e) => {
    const twice = isSingleSmallCaveVisitedTwice || vertex.isSmallCave && visited.has(vertex)
    return acc + numberOfPathsToEnd(e, new Set([...visited, vertex]), twice)
  }, 0)
}
